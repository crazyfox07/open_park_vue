<template>
  <div class="container-carsdel">
    ...
    <el-table border stripe :data="dataList">
        ...
        <el-table-column label="删除时间" prop="logTime"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async queryList () {
      this.dealFormData()
      const { data: res } = await this.$http.get(this.baseUrl + 'queryDelOrderList', {
    ...
    exportList () {
      if (this.start === null && this.end === null && this.queryInfo.plateNo === '') {
        return this.$message.error('操作日期和车牌号码不能同时为空')
      }
      if (this.queryInfo.plateNo === '') {
        if (this.start === null || this.end === null) {
          return this.$message.error('车牌号码为空，操作日期起止不能为空')
        }
      ...
      // console.log(params)
      window.location.href = this.$store.state.baseUrl + 'exportCarsdelList?' + params
    }
