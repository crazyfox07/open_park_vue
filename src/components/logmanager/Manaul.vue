<template>
  <div class="container-manaul">
    ...
    <el-table border stripe :data="dataList">
        ...
        <el-table-column label="抬杆时间" prop="logTime"></el-table-column>
        ...
        <el-table-column label="操作" width="120px">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-picture" size="mini" @click="showImgDialogVisible(scope.row.imgPath)">查看图片</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
     ...
     <el-dialog title="抬杆图片" :visible.sync="imgDialogVisible" width="40%" @close="imgDialogClose">
...
export default {
  ...
  methods: {
    ...
    async queryList () {
      this.dealFormData()
      const { data: res } = await this.$http.get(this.baseUrl + 'queryPoleList', {
    ...
    exportList () {
      ...
      // console.log(params)
      window.location.href = this.$store.state.baseUrl + 'exportManaulPoleList?' + params
    }
