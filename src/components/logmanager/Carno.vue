<template>
  <div class="container-sss">
    ...
    <el-table border stripe :data="dataList">
        ...
        <el-table-column label="操作时间" prop="optTime"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async queryList () {
      this.dealFormData()
      const { data: res } = await this.$http.get(this.baseUrl + 'queryCarnoList', {
    ...
    exportList () {
      ...
      // console.log(params)
      window.location.href = this.$store.state.baseUrl + 'exportCarnoList?' + params
    }
