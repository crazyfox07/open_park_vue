  ...
  <div class="container-leaguager">
    <el-card>
      <el-row>
        ...
        <!-- <el-col :span="4">
          <el-select placeholder="请选择订单状态" v-model="queryInfo.valState" class="el-input_query">
    ...
    <el-table border stripe :data="dataList">
        ...
        <el-table-column label="操作" prop="failImports" width="200px">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-s-tools" size="mini" @click="showxqDialogVisible(scope.row.vipId,scope.row.validBegintime,scope.row.expiryDate)">调整</el-button>
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteLeaguer(scope.row.vipId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
    ...
    addLeaguerInfo () {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        this.$confirm('确定添加该会员信息？', '提示', {
          ...
          const { data: res } = await this.$http.get(this.$store.state.baseUrl + 'addLeaguerInfo', {
            params: this.addForm
          })
          if (res.code === '0') {
