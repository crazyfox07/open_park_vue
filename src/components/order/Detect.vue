<template>
  <div class="container-detect">
    <el-card>
      <el-row>
        <el-col :span="5">
          <el-date-picker v-model="start" type="datetime" placeholder="过车时间期起"></el-date-picker>
        </el-col>
        <el-col :span="5">
          <el-date-picker v-model="end" type="datetime" placeholder="过车时间止"></el-date-picker>
        </el-col>
        <el-col :span="4">
          <el-input placeholder="请输入车牌号码" v-model="queryInfo.plateNo" />
        </el-col>
        <el-col :span="4">
            <el-select placeholder="方向" v-model="queryInfo.direction" style="width: 100%">
              <el-option label="全部" value=""></el-option>
              <el-option label="驶入" value="9"></el-option>
              <el-option label="驶离" value="10"></el-option>
            </el-select>
        </el-col>
        <el-col :span="4">
    ...
    <el-table border stripe :data="dataList">
        ...
        <el-table-column label="过车时间" prop="driveTime"></el-table-column>
        <el-table-column label="过车方向">
          <template slot-scope="scope">
            <span v-if="scope.row.direction === '9'  ">入场</span>
            <span v-if="scope.row.direction === '10'  ">离场</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="120px">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-picture" size="mini" @click="showImgDialogVisible(scope.row.pic)">查看图片</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    async queryList () {
      this.dealFormData()
      const { data: res } = await this.$http.get(this.baseUrl + 'queryDetectList', {
    ...
    exportList () {
      ...
      if (this.start === null && this.end === null && this.queryInfo.plateNo === '') {
        return this.$message.error('过车日期和车牌号码不能同时为空')
      }
      if (this.queryInfo.plateNo === '') {
        if (this.start === null || this.end === null) {
          return this.$message.error('车牌号码为空，过车日期起止不能为空')
        }
        const days = getNumberOfDays(this.start, this.end)
        console.log(days)
        if (days > 1) {
          return this.$message.error('过车日期起止时间差不能超过一天')
        }
      }
      this.dealFormData()
      // params格式化
      ...
      // console.log(params)
      window.location.href = this.$store.state.baseUrl + 'exportDetectList?' + params
    }
